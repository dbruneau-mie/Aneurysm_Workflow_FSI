...
# My environment (Should work if we don't switch back and forth)
...
python $SCRATCH/Aneurysm_Workflow_FSI/postprocessing_video/PlotAneuModeShapeBands.py $case_path $mesh_path $CameraPosition $CameraFocalPoint $CameraViewUp $CameraParallelScale $bands $r_sphere 1045 $glyph_length $glyph_density  # 1045 is from 2.753s, the start of the mode shape file
python $SCRATCH/Aneurysm_Workflow_FSI/postprocessing_video/PlotAneuModeShapeRandomVib.py $case_path $mesh_path $CameraPosition $CameraFocalPoint $CameraViewUp $CameraParallelScale $bands $r_sphere 1045 $glyph_length $glyph_density  # 1045 is from 2.753s, the start of the mode shape file
