...
then
    # Solid Sampling
    python $PROJECT/Aneurysm_Workflow_FSI/postprocessing_h5py/find_centroid.py --case=$case_path --mesh=$mesh_path --dt=$dt --start_t=$start_t_sd2 --end_t=$end_t_sd2 --save_deg=1 --stride=$stride_sd1 --bands="25,1400" --sampling_region="domain" --fluid_sampling_domain_ID=1 --solid_sampling_domain_ID=1001
    echo "Found Surgical Cases"
else
    # Sphere
    python $PROJECT/Aneurysm_Workflow_FSI/postprocessing_h5py/find_centroid.py --case=$case_path --mesh=$mesh_path --dt=$dt --start_t=$start_t_sd2 --end_t=$end_t_sd2  --save_deg=1 --stride=$stride_sd1 --bands="25,1400" --r_sphere=$r_sphere --x_sphere=$x_sphere --y_sphere=$y_sphere --z_sphere=$z_sphere 
