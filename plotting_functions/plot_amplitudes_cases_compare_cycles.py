...
simDir="/scratch/s/steinman/dbruneau/7_0_Surgical/Steady_Ramp_Cases_FC_Qavg"
ylim_v = 0.3
ylim_d = 1.5
end_t= 3.65 #3.804
plt_name="_qavg"
...
for i in range(len(vel_csv_files)):
    ...
    for dvp in dvp_list:
        ...
        cycle4=output_amplitudes[output_amplitudes[:,0]>=2.853]
        cycle4=cycle4[cycle4[:,0]<=end_t]
        cycle3=output_amplitudes[output_amplitudes[:,0]>=2.853-0.951]
        cycle3=cycle3[cycle3[:,0]<=end_t-0.951]
        
        print("Cycle 3: "+case_name)
        print("Temporal max, 99th percentile value: {}".format(np.max(cycle3[:,10])))
        print("Temporal min, 99th percentile value: {}".format(np.min(cycle3[:,10])))
        print("Temporal mean, 99th percentile value: {}".format(np.mean(cycle3[:,10])))
        print("Time of max value: {}".format(cycle3[np.argmax(cycle3[:,3]),0]))
        print("Cycle 4: "+case_name)
        print("Temporal max, 99th percentile value: {}".format(np.max(cycle4[:,10])))
        print("Temporal min, 99th percentile value: {}".format(np.min(cycle4[:,10])))
        print("Temporal mean, 99th percentile value: {}".format(np.mean(cycle4[:,10])))
        print("Time of max value: {}".format(cycle4[np.argmax(cycle4[:,3]),0]))
...
amp_graph_file=simDir+'/99th_percentile_amplitudes_all_cases'+plt_name+'.png' # file name for amplitudes
fig.tight_layout()
