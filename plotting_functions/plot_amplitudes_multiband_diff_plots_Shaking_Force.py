...
start_t=0.0 # start time of input data
end_t=1.0 # end time of input data
offset_t = 0.1
offset_end = end_t-start_t-0.05

simDirPuls="/scratch/s/steinman/dbruneau/7_0_Surgical/Modal_Shaking_Force"
simDirSurg="/scratch/s/steinman/dbruneau/7_0_Surgical/Surgical_Cases/Modal_Shaking_Force"
...
for idx, case_name in enumerate(case_list):
    ...
    for idy, band in enumerate(bands_list_plot):
        ...
        if band_plot_names[idy] == "Bruit":
            Bruit_max_amp.append(np.average(output_amplitudes[:,3]))
            Bruit_p99th_amp.append(np.average(output_amplitudes[:,10]))
        elif band_plot_names[idy] == "Mode 1":
            Mode1_max_amp.append(np.average(output_amplitudes[:,3]))
            Mode1_p99th_amp.append(np.average(output_amplitudes[:,10]))
        elif band_plot_names[idy] == "Mode 2":
            Mode2_max_amp.append(np.average(output_amplitudes[:,3]))
            Mode2_p99th_amp.append(np.average(output_amplitudes[:,10]))
        elif band_plot_names[idy] == "Mode 3":
            Mode3_max_amp.append(np.average(output_amplitudes[:,3]))
            Mode3_p99th_amp.append(np.average(output_amplitudes[:,10]))
            Case_Names.append(case_name)
        print("point with max amplitude during this timeframe : {}".format(int(output_amplitudes[np.argmax(output_amplitudes[:,3]),12])))
        print(np.average(output_amplitudes[:,3]))
        #print(output_amplitudes[np.argmax(output_amplitudes[:,3]),3])

          
    ax.set_xlabel('Simulation Time (s)')
    amp_graph_file=outDir+'/'+case_name+'_shaking_force_max_amplitudes_bands.png' # file name for amplitudes
    #fig.tight_layout()
    plt.savefig(amp_graph_file)  
    plt.close()
    
columns = ["Bruit time average, spatial Max Amplitude","Bruit time average, spatial 99th Percentile Amplitude",
           "Mode 1 time average, spatial Max Amplitude","Mode 1 time average, spatial 99th Percentile Amplitude",
           "Mode 2 time average, spatial Max Amplitude","Mode 2 time average, spatial 99th Percentile Amplitude",
           "Mode 3 time average, spatial Max Amplitude","Mode 3 time average, spatial 99th Percentile Amplitude","case name"]
...
csv_out_file=outDir+'/all_case_data_modal_amplitudes_shaking_force.csv' # file name for SBIs
print(df.head())
