...
#visualization_hi_pass_folder = "/media/db_ubuntu/Backup Plus/Results_Ramp/Case9_m047_predeformed/New/Visualization_hi_pass"
...
#bands="25,100000,95,140,345,375,470,485,485,510"
...
for i in range(num_bands):
    ...
    for dvp in dvp_list:
        ...
        ax[index].plot(output_amplitudes[:,0],output_amplitudes[:,10],label="{} Hz to {} Hz".format(lower_freq[i],higher_freq[i]))
        ax[index].set_ylabel(label)
        ax[index].legend()

          
ax[index].set_xlabel('Simulation Time (s)')
