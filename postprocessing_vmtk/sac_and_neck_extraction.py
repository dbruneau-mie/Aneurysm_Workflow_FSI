...
# python postprocessing_vmtk/sac_and_neck_extraction.py ../../../../../scratch/s/steinman/dbruneau/7_0_Surgical/Surgical_Cases/Pulsatile_CFD/MCA12_4D032/MCA12_4D032.config
...
case_path = os.path.dirname(config_file)
case_name = os.path.basename(case_path)

#print(os.listdir(case_path))

mesh_name = os.path.normpath(config.get("Case_Specific_Variables","mesh_path").strip('\"'))
mesh_name_domains = mesh_name+"_domains000000.vtu"
mesh_path = os.path.join(case_path,"mesh",mesh_name_domains)
...
csv_out_file = mesh_path.replace("domains000000.vtu","neck_area_and_volume.csv")
...
vol.Mesh=mesh
vol.Execute()
mesh_volume = vol.Volume
txt_out.append("Mesh volume is {} (in mm^3)\n".format(mesh_volume))
out_data["mesh_volume"] = [mesh_volume]
...
out_data["sac_volume"] = [sac_volume]
# Calculate neck area
...
out_data["neck_diameter"] = [neck_diameter]
...
out_data["aspect_ratio"] = [aspect_ratio]
