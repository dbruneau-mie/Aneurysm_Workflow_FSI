...
for i in range(num_frames):
    ...
    image_path = os.path.join(image_folder, 'pyvista_mode_shape_dark_aneu_edge_color_arrows'+viz_type+'_timestep_'+str(ts)+"_warp_"+str(warp_factor)+'.png')
    ...
    if i == 0:
        ...
        points_array = original_point_location + u_vec[node_max]*warp_factor
    else:
        u_vec_node_max = np.append(u_vec_node_max,u_vec[node_max])
        points_array = np.append(points_array,original_point_location + u_vec[node_max]*warp_factor)
    if i==1:
        lines_array = [2,0,1]
    ...
    plotter.add_mesh(points_plot,line_width=5.0,color="orange")#,point_size=1,render_points_as_spheres=True,color="orange")
    plotter.add_points(currentPoint,point_size=5,render_points_as_spheres=True,color="orange")
