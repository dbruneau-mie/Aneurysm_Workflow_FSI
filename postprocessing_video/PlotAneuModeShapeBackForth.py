...
for low_freq,high_freq,mode_color in zip(lower_freq,higher_freq,color_list):
    ...
    if high_freq - low_freq < 100:
        ...
        image_path = os.path.join(image_folder, 'pyvista_mode_shape_outline_pos_2_'+band_name_str+'_timestep_'+str(ts_pos)+"_warp_"+str(warp_factor)+'.png')
        ...
        plotter.add_mesh(outer_surf_undeformed, color="black", silhouette=silhouette_outer_echo2, opacity=0.0,lighting=True,smooth_shading=True, specular=0.00, diffuse=0.9, ambient=0.5)

        plotter.add_mesh(outer_surf_pos, color=mode_color, silhouette=silhouette_outer, opacity=1.0, lighting=True,smooth_shading=True, specular=0.00, diffuse=0.9, ambient=0.5)
